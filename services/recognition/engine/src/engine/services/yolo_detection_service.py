...
class YoloDetectionService(BaseRecognitionService):
    ...
    def __init__(self, models_dir: Path | None = None) -> None:
        ...
        self._models_dir = (
            models_dir or Path(config.MODELS_DIR).resolve() / "yolo" / "detection"
        )
    ...
    def load_model(self, model_name: str) -> YOLO:
        ...
        if not pt_file.exists():
            raise FileNotFoundError(f"YOLO detection model not found: {pt_file}")
    ...
    async def recognize(self, request: RecognitionRequest) -> RecognitionResponse:
        ...
        logger.info(
            "YOLO detection: model=%s, count=%d",
