...
系统拥有唯一的平台所有者。首次通过 `create_super_admin` 创建的账号会成为平台所有者；
已有数据库升级时，最早创建且仍启用的超级管理员会自动成为平台所有者。

- 平台所有者可以任命、降级、禁用和重置其他超级管理员。
- 普通超级管理员可以审批和管理普通用户，但不能修改任何管理员角色，也不能管理其他超级管理员。
- 平台所有者账号不能从用户管理接口降级、禁用或重置密码；所有者应从个人修改密码页面维护自己的密码。

平台所有者和超级管理员在标注、模型及综合检测业务数据上仍具有相同的全局管理权限；这个
层级只用于保护管理员账号本身。
